...
![種まき、森の成長、鳥の訪問、夜、再生成した島](preview.png)
...
| Z / 地面をクリック | 空き地に種まき、成木は間引き、枯れ木は片付け |
| X / 右クリック | 島全体に雨を降らせる |
| R | 輪郭・池・川・木の配置が異なる島を生成 |
| 画面下の PLANT（THIN / CLEAR）/ RAIN / NEW ISLAND | 同じ操作をマウスで行う |
...
成木が周囲8マスに5本以上あると密集のストレスがたまり、葉が黄褐色になって
枯れます。雨を降らせても密集は解消しません。Zで木を間引くと、隣の木は
回復できます。乾燥が長く続いた場合も衰弱しますが、こちらは雨で回復します。

寿命を迎えた木も枯れ、枯れ木は6〜8秒で保水力のある土に戻ります。
そこへ種が広がり、また成長するので、森が埋まりきって終わることはありません。
枯れ木をZで片付ければ早く植え直せます。木ごとに寿命は異なります。

時々鳥が訪れて、木に留まり、離れた空き地に種を運んで飛び去ります。
訪問中は鳴き声も聞こえます。雨の最中や元気な成木がないときには訪れません。
鳥が運んだ種も水が必要です。

森が増えると虫が集まり、8本、12本、16本で音楽のパートが増えます。
旋律は木の座標から五音音階で作り、48秒で昼と夜を一周します。
森が衰退すれば音楽のパートも減ります。
庭の保存はありません。Rは次の地形を生成し、デモの切り替えでは最初に戻ります。
最初の島とRで巡る島の順序は固定シードなので、同じ操作を再現できます。

## このプロジェクトで作る理由

READMEの「三つの時計」を、ひとつの場所にしました。

- **TypeScriptの世界**：地形生成、水分、成長と衰退、鳥による種の拡散、木の配置から作る旋律。
- **V9938の風景**：SCREEN 5の16色。変化した風景を本来の速度のブリッターで
  裏ページへ描き、完成してから表示。待ち仕事を積み増さず、次は最新の状態を描きます。
- **60Hzの生き物**：選択カーソル、鳥、虫、雨はハードウェアスプライト。
...
最初の島、種まき後、雨で成長した森、鳥の訪問、夜、次の島の6枚を並べたPNGと、
同じ約24秒間のエミュレーターの音をWAVに出力します。WAVの引数は省略できます。
乱数は固定シードなので同じ操作から同じ庭が育ちます。

`garden.ts` が描画に依存しない成長規則、`demo.ts` が画面・操作・音です。
`test/seed.test.ts` では地形の再現性・連結性、密集と乾燥からの回復、枯死と再成長、
鳥の種運び、5分間の循環、遅い描画中の操作と島の再生成を確認しています。
